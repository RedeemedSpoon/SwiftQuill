...
class SwiftQuill(Tk):
    ...
    def score_tab(self, event=None):
        if self.open_tab != 'Score':
            ...
            if df.shape[0] < 2:
                Label(self, text="No Scores Yet!", font=("Noto Sans", 25)).grid(row=1, column=2, pady=(0,20))
                Label(self, text="You must start at least 3 tests", font=("Noto Sans", 15)).grid(row=2, column=2)
            else:
                Label(self, text=f"Progress Since {df['Date'][1].split(' ')[0]}", font=("Noto Sans", 25)).grid(row=1, column=2,columnspan=2, pady=(0,30))
                self.graph = ImageTk.PhotoImage(Image.open("./Assets/graph.png"))
                Label(self, image=self.graph).grid(row=2, column=2, columnspan=2, pady=(0,15))
                Label(text=f"Average WPM: { round(df['WPM'].mean(), 2) }", font=("Noto Sans", 15)).grid(row=3, column=2)
    ...
    def setting_tab(self, event=None):
        if self.open_tab != 'Settings':
            ...
            self.clear()
            Label(self, text="Settings Options", font=("Noto Sans", 25)).grid(row=1, column=2, columnspan=3, pady=(0,30))

            self.op_language = StringVar(value=self.language)
            Label(self, text='Language', font=("Noto Sans", 15)).grid(row=2, column=2, padx=10, pady=10)
            ttk.Combobox(self, values=self.lang_list, width=15, textvariable=self.op_language).grid(row=2, column=3, columnspan=2, padx=10, pady=10)

            self.op_tracking = BooleanVar(value=self.tracking)
            Label(self, text='Progress Tracking', font=("Noto Sans", 15)).grid(row=3, column=2, padx=10, pady=10)
            Radiobutton(self, text="Yes", variable=self.op_tracking, value=True, cursor='hand2', font=("Noto Sans", 12)).grid(row=3, column=3, padx=10, pady=10)
            Radiobutton(self, text="No", variable=self.op_tracking, value=False, cursor='hand2', font=("Noto Sans", 12)).grid(row=3, column=4, padx=10, pady=10)

            self.op_literature = BooleanVar(value=self.literature)
            Label(self, text='Text Type', font=("Noto Sans", 15)).grid(row=4, column=2, padx=10, pady=10)
            Radiobutton(self, text="Literature Writing", variable=self.op_literature, value=True, cursor='hand2', font=("Noto Sans", 12)).grid(row=4, column=3, padx=10, pady=10)
            Radiobutton(self, text="Research Paper Writing", variable=self.op_literature, value=False, cursor='hand2', font=("Noto Sans", 12)).grid(row=4, column=4, padx=10, pady=10)

            self.op_test_length = IntVar(value=self.test_length)
            Label(self, text='Test Length (Seconds)', font=("Noto Sans", 15)).grid(row=5, column=2, padx=10, pady=10)
            ...
            Label(self, text='Graph Mean (Simplicity)', font=("Noto Sans", 15)).grid(row=6, column=2, padx=10, pady=10)
            ...
            Button(self, text="Save", font=("Noto Sans", 15), command=self.save_settings).grid(row=7, column=2, columnspan=3, pady=50)
    ...
    def add_data(self):
        if self.key_pressed > 15:
            ...
            self.timer.configure(text="Accuracy: " + str(self.accuracy) + "% | WPM: " + str(self.WPM))
            if self.tracking:
